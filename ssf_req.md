...
### 1.1 시스템 목적
스마트공방의 제품, 재고, 거래처, 주문, 생산, 출하, KPI를 통합 관리하는 웹 기반 MES(Manufacturing Execution System)
...
| 항목 | 내용 |
|------|------|
| 그룹 구조 | `nav-group` 토글 방식 (클릭 시 펼침/접기) |
| 화살표 | ▾ 아이콘, 펼침 시 180° 회전 애니메이션 |
| 하위 항목 | 들여쓰기 + `─` prefix |
| 자동 펼침 | KPI 하위 페이지 이동 시 그룹 자동 펼침 |
...
| 항목 | 내용 |
|------|------|
| 동작 | settings 테이블에서 `kpi_%` 키 조회 |
| 응답 | `{ kpi_pi_target: 95, kpi_pi_warning: 85, ... }` |

#### 설정 저장 (PUT /api/kpi/settings)

| 항목 | 내용 |
|------|------|
| 동작 | `kpi_` 접두사 키만 INSERT OR REPLACE |
| 모달 | 5개 지표(PI, QI, Yield, 불량률, 폐기률) × 3단계(목표, 경고, 위험) |
...
| 상태 | 색상 | 조건 (일반) | 조건 (inverted) |
|------|------|-------------|-----------------|
| good | 초록 (#2ecc71) | value ≥ target | value ≤ target |
| warning | 주황 (#f39c12) | value ≥ warning | value ≤ warning |
| danger | 빨강 (#e74c3c) | value < warning | value > warning |

### 10.3 생산성 KPI 페이지

#### 생산성 조회 (GET /api/kpi/productivity)

| 항목 | 내용 |
|------|------|
| 메뉴 | KPI 관리 > 생산성 |
| 헤더 버튼 | "임계치 설정" |
| 파라미터 | start_date, end_date, product_id (모두 선택) |
| 데이터 소스 | productions 테이블 (status='완료', completed_at IS NOT NULL) |
| 계산 공식 | PI = actual_qty / planned_qty × 100 |
...
| 제품 필터 | 드롭다운 (전체 제품 / 개별 선택) |
| 조회 버튼 | 필터 적용 후 데이터 리로드 |
| 스냅샷 | 날짜 입력 + "스냅샷 생성" 버튼 (우측) |

**KPI 요약 카드 (3개):**

| 카드 | 값 | 색상 |
|------|-----|------|
| 생산지수 (PI) | {pi}% + 프로그레스바 | good/warning/danger |
| 총 실적 수량 | {total_actual} (하위: 계획 수량) | 파란색 |
| 완료 건수 | {production_count} | 파란색 |

**일별 생산지수 테이블:**

| 컬럼 | 내용 |
|------|------|
| 날짜 | DATE(completed_at) |
| 완료 건수 | COUNT |
| 실적 수량 | SUM(actual_qty) |
| 계획 수량 | SUM(planned_qty) |
| PI (%) | 임계치 색상 적용 |
...
| 지표 | 공식 |
|------|------|
| QI (품질지수) | (actual - defect) / actual × 100 |
| Yield (수율) | (actual - defect - waste) / actual × 100 |
| Defect Rate (불량률) | defect / actual × 100 |
| Waste Rate (폐기률) | waste / actual × 100 |

**KPI 요약 카드 (4개):**

| 카드 | 색상 방향 | 프로그레스바 |
|------|-----------|-------------|
| 품질지수 (QI) | 일반 (높을수록 good) | width = min(qi, 100)% |
| 수율 (Yield) | 일반 | width = min(yield, 100)% |
| 불량률 | inverted (낮을수록 good) | width = min(rate×10, 100)% |
| 폐기률 | inverted | width = min(rate×10, 100)% |

**일별 품질지수 테이블:**

| 컬럼 | 내용 |
|------|------|
| 날짜, 건수, 실적, 불량, 폐기 | 기본 수량 |
| QI (%), 수율 (%), 불량률 (%), 폐기률 (%) | 각각 임계치 색상 적용 |

**제품별 품질지수 테이블:**

| 컬럼 | 내용 |
|------|------|
| 제품코드, 제품명, 실적, 불량, 폐기 | 기본 수량 |
| QI (%), 수율 (%), 불량률 (%), 폐기률 (%) | 각각 임계치 색상 적용 |
...
|------|------|
| 요청 | `{ date }` (기본: 오늘) |
| 동작 | 해당 날짜 완료 생산 → 제품별 집계 → kpi_daily INSERT OR REPLACE |
| 응답 | `{ message, count }` |
| 데이터 없음 | count=0, 경고 토스트 |
...
|------|------|
| 파라미터 | start_date, end_date, product_id (모두 선택) |
| 응답 | kpi_daily 레코드 배열 (product_name, product_code JOIN) |
| 정렬 | 날짜 DESC, 제품명 ASC |
...
### 11.3 출하 현황 (GET /api/reports/shipment/daily)
...
### 11.5 재고 현황 (GET /api/reports/inventory/status)
...
### 12.2 기본 설정 (GET/POST /api/settings)
...
| kpi_daily | KPI 일별 스냅샷 | → companies, products |
...
|------|------|----------|------|
| id | INTEGER | PK, AUTOINCREMENT | |
| date | DATE | NOT NULL | 스냅샷 날짜 |
| company_id | INTEGER | FK→companies | 회사 ID |
| product_id | INTEGER | FK→products | 제품 ID |
| pi | REAL | DEFAULT 0 | 생산지수 (%) |
| qi | REAL | DEFAULT 0 | 품질지수 (%) |
| yield_rate | REAL | DEFAULT 0 | 수율 (%) |
| defect_rate | REAL | DEFAULT 0 | 불량률 (%) |
| waste_rate | REAL | DEFAULT 0 | 폐기률 (%) |
| actual_qty | INTEGER | DEFAULT 0 | 실적 수량 |
| planned_qty | INTEGER | DEFAULT 0 | 계획 수량 |
| defect_qty | INTEGER | DEFAULT 0 | 불량 수량 |
| waste_qty | INTEGER | DEFAULT 0 | 폐기 수량 |
| production_count | INTEGER | DEFAULT 0 | 생산 건수 |
| created_at | DATETIME | DEFAULT CURRENT_TIMESTAMP | 생성일시 |

> UNIQUE 제약: (date, company_id, product_id)
...
| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | /api/kpi/productivity | 생산성 KPI 실시간 계산 (필터: start_date, end_date, product_id) |
| GET | /api/kpi/quality | 품질 KPI 실시간 계산 (필터: start_date, end_date, product_id) |
| GET | /api/kpi/settings | KPI 임계치 설정 조회 |
| PUT | /api/kpi/settings | KPI 임계치 설정 저장 |
| POST | /api/kpi/snapshot | 일별 KPI 스냅샷 생성 |
| GET | /api/kpi/snapshots | KPI 스냅샷 이력 조회 |
...
### 16.1 레이아웃
- **사이드바**: 좌측 고정 240px, 다크 테마, 10개 메뉴 + KPI 서브메뉴 그룹 (회원관리는 조건부 표시)
